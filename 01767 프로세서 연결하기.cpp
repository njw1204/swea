...
void dfs(int node, int curConn, int curSize) {
    const pi& core = cores[node];
    bool isLast = (node == cores.size() - 1);
    vector<pi> dxs = {{-1, 0}, {1, 0}, {0, -1}, {0, 1}};

    if (isLast) {
        ans = min(ans, {-curConn, curSize});
    }
    else {
        int maxConn = curConn + cores.size() - node;

        if (make_pair(-maxConn, INT_MIN) > ans) {
            return;
        }

        dfs(node + 1, curConn, curSize);
    }
    ...
    for (const pi& dx : dxs) {
        ...
        while (cur.$1 >= 1 && cur.$1 <= N && cur.$2 >= 1 && cur.$2 <= N) {
            if (field[cur.$1][cur.$2] != 0) {
                ok = false;
                break;
            }
        ...
        if (isLast) {
            ans = min(ans, {-(curConn + 1), curSize + size});
            continue;
        }
        ...
        dfs(node + 1, curConn + 1, curSize + size);
