...
pair<int, int> querySeg(int queryLeft, int queryRight, int node, int start, int end) {
    ...
    pair<int, int> left = querySeg(queryLeft, queryRight, node * 2, start, (start + end) / 2);
    pair<int, int> right = querySeg(queryLeft, queryRight, node * 2 + 1, (start + end) / 2 + 1, end);
    return make_pair(max(left.$1, right.$1), min(left.$2, right.$2));
}
    ...
    TESTCASE(T) {
        ...
        REP(_, q) {
            ...
            if (t == 0) {
                ...
                DINPUT(int, l, r);
                pair<int, int> result = querySeg(l, r - 1, 1, 0, n - 1);
