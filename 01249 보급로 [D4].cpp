...
int main() {
    ...
    TESTCASE(T) {
        ...
        while (!pq.empty()) {
            ...
            for (pi dx : dxs) {
                pi nextPos = {cur.$2.$1 + dx.$1, cur.$2.$2 + dx.$2};

                if (field[nextPos.$1][nextPos.$2] < 0) {
                    continue;
                }

                int nextDist = cur.$1 + field[nextPos.$1][nextPos.$2];

                if (dist[nextPos.$1][nextPos.$2] == -1 || nextDist < dist[nextPos.$1][nextPos.$2]) {
                    dist[nextPos.$1][nextPos.$2] = nextDist;
                    pq.push({nextDist, nextPos});
                }
            }
        }

        cout << "#" << T << " " << dist[N][N] << endl;
    }
}
