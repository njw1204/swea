...
int main() {
    ...
    TESTCASE(T) {
        ...
        REP(_, N) {
            int base = !X.empty() ? X.back().$2 : 0;
            DINPUT(int, s, e);
            e--;
            X.push_back({{s, 0}, base + 1});
            X.push_back({{e, 1}, base + e - s + 1});
        }
        ...
        for (int i = 0; i < X.size(); i++) {
            ...
            int from = X[i].$1.$1;
            int to = from + L - 1;
            int endIdx = upper_bound(X.begin() + prevEndIdx, X.end(), make_pair(make_pair(to, INT_MAX), INT_MAX)) - X.begin() - 1;

            ans = max(ans, X[endIdx].$2 + (X[endIdx].$1.$2 == 0 ? to - X[endIdx].$1.$1 : 0) - (i > 0 ? X[i - 1].$2 : 0));
            prevEndIdx = endIdx;
        }
