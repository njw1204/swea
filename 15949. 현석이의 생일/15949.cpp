...
bool solve() {
    ...
    while (true) {
        ...
        if (sufficient) {
            ...
            for (int i = 0; i < N.size() - ans.size(); i++) {
                saved.push_back(y);
            }
        ...
        if (y <= N[ans.size()] - '0') {
            sufficient |= y < N[ans.size()] - '0';
            lastYPos = ans.size();
            ans.push_back(y);
            continue;
        }

        if (x <= N[ans.size()] - '0' && (ans.size() != 0 || x != 0)) {
            sufficient |= x < N[ans.size()] - '0';
            ans.push_back(x);
            continue;
        }
        ...
        if (lastYPos != -1) {
            ...
            for (int i = 0; i < lastYPos; i++) {
                saved.push_back(ans[i]);
            }

            saved.push_back(x);

            for (int i = 0, cnt = N.size() - saved.size(); i < cnt; i++) {
                saved.push_back(y);
            }
