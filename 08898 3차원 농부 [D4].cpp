...
vector<int> cows, horses;

int main() {
    UNSYNC; READTXT;
    cows.reserve(500000);
    horses.reserve(500000);

    TESTCASE(T) {
        cows.clear();
        horses.clear();

        DINPUT(int, N, M, C1, C2);
        ...
        for (int cow : cows) {
            ...
            while (true) {
                int horseIdxLeft = ptr;
                int cur = abs(cow - horses[ptr]);
        ...
        cout << "#" << T << " " << ans + abs(C1 - C2) << " " << ansCnt << endl;
    }
}
