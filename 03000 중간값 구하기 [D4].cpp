...
int main() {
    ...
    TESTCASE(T) {
        ...
        priority_queue<ll> leftMaxPq;
        priority_queue<ll, vector<ll>, greater<>> rightMinPq;
        DINPUT(ll, N, A);
        rightMinPq.push(A);
        ...
        REP(_, N) {
            ...
            while (leftMaxPq.size() > rightMinPq.size() - 1) {
                rightMinPq.push(leftMaxPq.top());
                leftMaxPq.pop();
            }

            while (rightMinPq.size() - 1 > leftMaxPq.size()) {
                leftMaxPq.push(rightMinPq.top());
                rightMinPq.pop();
            }

            ans = (ans + rightMinPq.top()) % 20171109;
        }
